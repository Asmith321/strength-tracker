...
console.log("\n== AUDIT 2.10: feeler steps track priming — cold gets 2, primed gets 1 ==");
{
  ...
  const items = prescribe(p, green).items;
  const inclinebench = items.find((i) => i.key === "inclinebench"); // chest, COLD — the day's first chest work
  ...
  check(`cold accessory (inclinebench, the day's first chest work) gets a 2-step feeler (${inclinebench.warmup?.sets?.length} steps)`,
    inclinebench.warmup?.sets?.length === 2 && inclinebench.warmup.sets[0].weight < inclinebench.warmup.sets[1].weight);
  ...
  check("sanity: dip really is primed by an earlier same-muscle exercise",
    LIB.dip.volumeGroup === LIB.inclinebench.volumeGroup
    && items.findIndex((i) => i.key === "inclinebench") < items.findIndex((i) => i.key === "dip"));
  ...
  for (const it of [inclinebench, dip, latpullover])
